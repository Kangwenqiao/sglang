...
class TestCliChoicesAreASubset(CustomTestCase):
    """A registry key is not a `--quantization` value.

    A method that requires an already-quantized checkpoint is reached only
    through that checkpoint's `quant_method`, so offering it on the CLI would
    accept a flag that cannot work. `blockwise_int8` is one:
    `BlockInt8LinearMethod` asserts both a `weight_block_size` and an
    int8-serialized checkpoint, and the former may only be set when the latter
    holds.
    """
